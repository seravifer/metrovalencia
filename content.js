$(document).ready(function() {
    ...
    $('#selectOrigen option[value="' + Cookies.get('cookieOrigen') + '"]').attr("selected", "selected");
    $('#selectDestination option[value="' + Cookies.get('cookieDestination') + '"]').attr("selected", "selected");
    $('#date').val(getDay());
    $('#hora').val(getTime());
    ...
    $('#invert').click(function() {
        /*var aux = $('#selectOrigen option:selected').val();
        $('#selectOrigen option[value="' + $('#selectDestination option:selected').val() + '"]').attr("selected", "selected");
        $('#selectDestination option[value="' + aux + '"]').attr("selected", "selected");/*/
    ...
    $('#form').submit(function(event) {
        ...
        $.ajax({
            url         : 'http://www.metrovalencia.es/horarios.mobi.php',
            type        : 'POST',
            data        : formData,
            encode      : true,
            dataType    : 'html'
    ...
    $('#selectOrigen').change(function() {
        Cookies.set('cookieOrigen', $(this).val(), {expires: 30});
    });
    
    $('#selectDestination').change(function() {
        Cookies.set('cookieDestination', $(this).val(), {expires: 30});
    });
